...
describe('Yale to Fale replacement logic', () => {
  ...
  test('should handle text that has no Yale references', () => {
    const htmlWithoutYale = `
      ...
      <body>
        <h1>Hello World</h1>
        <p>This is a test page with no Y*l* references.</p>
      </body>
      </html>
    `;
    ...
    expect(modifiedHtml).toContain('<p>This is a test page with no Y*l* references.</p>');
  });
  ...
  test('should handle case-insensitive replacements', () => {
    ...
    $('body *').contents().filter(function() {
      ...
      const text = $(this).text();
      // Use a replacer function to preserve case
      const newText = text.replace(/Yale/gi, function(match) {
        if (match === 'YALE') return 'FALE';
        if (match === 'yale') return 'fale';
        return 'Fale'; // Yale or any other case variation
      });
      if (text !== newText) {
        $(this).replaceWith(newText);
      }
